...
import { CreatePaymentsV3Command } from './commands/create-payment/create-payment.command';
...
import { RejectionReasonPaymentsV3Command } from './commands/rejection-reason/rejection-reason-payment.command';
import { RejectionReasonPaymentV3Dto } from './dto/rejection-reason-payment-v3.dto';
...
export class PaymentsV3Service {
  constructor(
    ...
    private readonly paymentRulesService: PaymentV3RulesService,
    @Inject(REQUEST) private readonly request: Request,
  ) {}

  async create(createPaymentsV3Dto: CreatePaymentsV3Dto) {
    const correlationId = this.request.correlationId;
    this.logger.debug('Hello world log', { correlationId });
    try {
      this.logger.log(
        `Payload de pagamento: ${JSON.stringify(createPaymentsV3Dto)}`,
      );
      ...
      if (dto.localInstrument !== 'MANU' && dto.localInstrument != 'INIC') {
        ...
        const dictData = await this.pixService.getDict(
          createPaymentsV3Dto.data.proxy,
          createPaymentsV3Dto.data.creditorAccount.cpfCnpj,
        );

        await this.paymentRulesService.validateDictData(
          createPaymentsV3Dto.data.creditorAccount,
          dictData,
        );
      }
      ...
      const command = plainToClass(CreatePaymentsV3Command, {
  ...
  async rejectionReasonUpdate(paymentId: string, code: string, detail: string) {
    try {
      ...
      const updatePayment: RejectionReasonPaymentV3Dto = {
        paymentId: paymentId,
        status: 'RJCT',
        rejectionReason: {
          code,
      ...
      const command = plainToClass(RejectionReasonPaymentsV3Command, {
        paymentId: paymentId,
        ...updatePayment,
      });
  ...
  private mapToPaymentV3ResponseDto(payment): any {
